...
$$.onPageLoad = function(_src) {	
	...
	if (_src) {
		...
		if($j.galleria.autoPlay){
			var tmFunc = function(){ $j.galleria.next(); };
			$j.galleria.timer = setTimeout(tmFunc, $j.galleria.slideDelay);
		}
		
		// add clickable image helper
		if($j.galleria.clickNext) {
			_img.css('cursor','pointer').click(function() {
		      if($j.galleria.autoPlay) clearTimeout($j.galleria.timer);
			  $j.galleria.next();
              });
		}
...
$j.extend({galleria : {
	...
	startShow : function() {
 	    $j.galleria.autoPlay = true;
        $j.galleria.next();
	},
	stopShow : function() {
 	    $j.galleria.autoPlay = false;
        clearTimeout($j.galleria.timer);
	}
}
});
